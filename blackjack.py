...
class BJ_Player(BJ_Hand):
    ...
    def betting(self):
        self.bet = 0
        while self.bet < 1 or self.bet > self.money:
            try:
                self.bet = int(input(self.name+", ile chcesz postawić?"))
            except:
...
class BJ_Game(object):
    ...
    def play(self):
        # jeśli jest za mało kart by grać dodaj komplet kart i przetasuj
        ...
        for player in self.players:
            if player.money == 0:
                print(player.name, "odpada z gry")
