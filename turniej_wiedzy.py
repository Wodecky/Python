...
def next_block(the_file):
    ...
    punktacja = next_line(the_file)
    explanation = next_line(the_file)

    return category, question, answers, correct, punktacja, explanation
...
def tablica_wyników(score):
    """Wyświetla tablicę wyników"""
    rekordy = shelve.open("rekordy.dat")
    imię = input("Jak się nazywasz?")
    imiona = rekordy["imiona"]
    punkty = rekordy["punkty"]
    ...
    rekordy["imiona"] = imiona
    rekordy["punkty"] = punkty

    print("Oto najlepsze wyniki:")

    for i in range(len(punkty)):
        print(imiona[i], "\t", punkty[i])
...
def main():
    ...
    category, question, answers, correct, punktacja, explanation = next_block(trivia_file)
    while category:
        # zadaj pytanie
        print(category)
        print(question)
